...
def init(opts):
    ...
    titlefontsize=int(opts.internaltitleheight*opts.titlefontfactor)

    fontantialias=opts.fontantialias
    fontantialiasbit=0
    if fontantialias:
        fontantialiasbit=1
    #true or false has to be set with 1 or 0
    cmd="""\
    xfconf-query -c xfwm4 -p /general/theme -s "cdetheme"
    xfconf-query -c xfwm4 -p /general/button_layout -s 'O|THCM'
    xfconf-query -c xfwm4 -p /general/title_font -s "{font} {fontStyle} {titlefontsize}"
    xfconf-query -c xfwm4 -p /general/placement_mode -s "center"
    xfconf-query -c xsettings -p /Xft/Antialias -s {fontantialiasbit}
    xfconf-query -c xsettings -p /Gtk/DialogsUseHeader -s 0
    """.format(**locals())
    #20221114 last line gets rid of these STUPID decorationless dialogs
